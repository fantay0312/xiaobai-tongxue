/**
 * 下课钤印 —— 复盘页的落印仪式条。
 * 一堂课新落的印章、师道晋级、小白科名晋阶,在批注开卷前先钤下来;
...
export function HonorRoll({ honors }: { honors: SessionHonors }) {
  ...
  // 学识进阶(升级轨)也是一件值得钤下的荣誉,单它成真也让钤印条现身
  if (newSeals.length === 0 && !promoted && !levelUp && !xpLevelUp) return null;

  const stageBefore = getStageMeta(honors.pupilLevelBefore);
  const stageAfter = getStageMeta(honors.pupilLevelAfter);
      ...
      <div className={s.row}>
          ...
          <div className={`${s.chip} ${s.rankChip}`} style={stamp(newSeals.length)}>
            ...
            <span className={s.body}>
              ...
              <span className={s.desc}>
                ...
                {/* 学识进账并入这条现成的进账句;若本课学识进了阶,进账改由下面的学识签细说,此处不重复 */}
        ...
        {levelUp && (
          <div className={`${s.chip} ${s.pupilChip}`} style={stamp(newSeals.length + (promoted ? 1 : 0))}>
            ...
            <span className={s.body}>
              <b className={s.name}>科名晋阶 · {stageAfter.name} · {stageAfter.description}</b>
              <span className={s.desc}>
                {stageBefore.name} → {stageAfter.name},是先生一课一课教出来的
              </span>
            </span>
          </div>
        )}
        {xpLevelUp && (
          ...
          >
            ...
            <span className={s.body}>
              <b className={s.name}>学识进阶 · 第 {honors.xpLevelAfter} 阶</b>
              <span className={s.desc}>
                学识 +{xpGained} 点,从第 {honors.xpLevelBefore} 阶进至第 {honors.xpLevelAfter} 阶
              </span>
            </span>
          </div>
        )}
