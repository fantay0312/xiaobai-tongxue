...
- 设计/浏览器 ✓ 线上桌面实测 42 知识星、6 突出星、28 装饰星；390×844 手机默认单课 6/4/14，操作系统 30 星以 5 列排布，最小中心距 52.65px、44px 点击目标、根级横向溢出 0。Token 星选择后出现 2 个语义邻星与 2 条星链，证据链同步展开；方向键焦点正确移动且全场仅 1 个 roving `tabIndex=0`；浏览器日志为空。设计对照与桌面/手机截图已保存到 `doc/qa-evidence/2026-07-30-star-sea/`，文件级 SHA-256 与发布来源记录在该目录 README，`design-qa.md` 结论为 passed。
...
- 秘密/发布树 ✓ PR head `404a9084aa11dbcc1c4851f8b2d5f29143824616` 与 squash commit `c93169e315be0659a0c5ba6d6db7a524f56fb5b1` 指向同一 Git tree `72a25750fdd9a47ad1de1467a961449267319105`；GitGuardian check run `90830122651` 对该 PR tree 检查成功。合并提交 clean build 当时扫描 226 个 `app/dist` 文件，真实本地 API key 精确值与通用 secret pattern 均为 0；发布 tar SHA-256 为 `b6944e329bb8605fc6f6f4dfce6bfc09f4f0a1c8250fde58a9f948f46767a4da`，不含 AppleDouble。原 226 文件公网比对 manifest 未入库，不能从 fresh checkout 独立恢复；现有可复验证据及其 SHA-256 收录于 `doc/qa-evidence/2026-07-30-star-sea/`，不再把缺失 manifest 描述成可复现附件。
...
- CDN/公网 ⓘ 当时操作记录显示腾讯 CDN purge `630612165304266818` 为 done，且公网 index、growth JS/CSS、银河 WebP、科名模块通过逐字节比对；由于原 226 文件 manifest 未入库，这项历史字节一致性结论现不能独立复验。仍可由记录定位的健康检查为：HTTPS HTTP/2 `/`、`/admin/`、`/api/me`、`/api/commerce/catalog` 200，API no-store，HTTP→HTTPS 和旧入口 308，绕过 CDN 直连源站 403；systemd active、只监听 `127.0.0.1:8000`、`NRestarts=0`，Nginx 语法与上线后 journal warning 检查通过。
